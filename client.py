...
class FedRecClient(nn.Module):
    def __init__(self, train_ind, test_ind, target_ind, m_item, dim, client_id):
        super().__init__()
    ...
    def eval_(self,items_emb, linear_layers):
        ...
        if self._test_:
            hr_at_10 = evaluate_recall(rating, self._test_, 10)
            hr_at_20 = evaluate_recall(rating, self._test_, 20)
            ndcg_at_10 = evaluate_ndcg(rating, self._test_, 10)
            ndcg_at_20 = evaluate_ndcg(rating, self._test_, 20)
            test_result = np.array([hr_at_10, hr_at_20, ndcg_at_10, ndcg_at_20])
