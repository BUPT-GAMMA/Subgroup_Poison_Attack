...
class FedRecServer(nn.Module):
    ...
    def train_(self, clients, batch_clients_idx):
        ...
        _ = torch.zeros((len(batch_clients_idx), len(self.items_emb.weight), self.dim))
        
        for idx in batch_clients_idx:
            client = clients[idx]
            #items是每个client交互的train items
    ...
    def view_recomms(self, clients, items_popularity):
        items_emb = self.items_emb.weight
        linear_layers = [(layer.weight, layer.bias) for layer in self.linear_layers]
        import numpy as np
        cid = np.random.randint(0, len(clients))
        clients[cid].view_recomms(items_popularity, items_emb, linear_layers)
    ...
    def eval_(self, clients):
        ...
        return test_results / test_cnt if test_cnt > 0 else 0.0,target_results / target_cnt if target_cnt > 0 else 0.0
